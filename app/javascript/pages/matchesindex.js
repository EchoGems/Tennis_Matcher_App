...
export default class MatchesIndex extends Component {
  ...
  render(){
    ...
    var matches = this.state.matches.map(function(el, i){
      ...
      if (matchdate >= today) {
        return (
            <MatchCard key={i} current_user={current_user} user_name={el.user_name} id={el.id} location={el.location} timeslot={el.timeslot} user2_id={el.user2_id} user_id={el.user_id} user_skill={el.user_skill} user2_name={el.user2_name} user2_skill={el.user2_skill} address={el.address} zip={el.zip}/>
        )
      }
    })
